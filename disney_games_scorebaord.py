...
houses = ["Inferno",  "Cyclones", "Comets",  "Lightning"] #house names
colors = ["red"       ,"green"    ,"yellow"    ,"cyan"] #respective colors for fonts
pic_width    = 600
pic_height   = 360
my_font      = "Harry P" #Waltograph
my_font_size = 140
...
if __name__ == "__main__":
  ...
  inferno_pic   = ImageTk.PhotoImage(inferno_pic)
  cyclones_pic  = Image.open("team_pics/cyclones.jpg")
  cyclones_pic  = cyclones_pic.resize((res_pic_width,res_pic_height),Image.Resampling.LANCZOS)
  cyclones_pic  = ImageTk.PhotoImage(cyclones_pic)
  comets_pic    = Image.open("team_pics/comets.jpg")
  comets_pic    = comets_pic.resize((res_pic_width,res_pic_height),Image.Resampling.LANCZOS)
  comets_pic    = ImageTk.PhotoImage(comets_pic)
  lightning_pic = Image.open("team_pics/lightning.jpg")
  lightning_pic = lightning_pic.resize((res_pic_width,res_pic_height),Image.Resampling.LANCZOS)
  lightning_pic = ImageTk.PhotoImage(lightning_pic)

  [scores,member_counts] = make_scoreboard(root,houses,colors,[TOP,BOTTOM],[inferno_pic,cyclones_pic,comets_pic,lightning_pic])
